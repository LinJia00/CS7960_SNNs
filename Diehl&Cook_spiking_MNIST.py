...
#TODO Remove
#myVar = np.arange(10)
#np.save(MNIST_data_path + '\\activity\\myFile', myVar)
...
if not test_mode:
    ...
    np.save(MNIST_data_path + '\\activity\\resultPopVecs' + str(num_examples), result_monitor)
    np.save(MNIST_data_path + '\\activity\\inputNumbers' + str(num_examples), input_numbers)
...
if rate_monitors:
    b.figure(fig_num)
    fig_num += 1
    for i + 1, name in enumerate(rate_monitors):
...
if spike_monitors:
    b.figure(fig_num)
    fig_num += 1
    for i + 1, name in enumerate(spike_monitors):
...
if spike_counters:
    b.figure(fig_num)
    fig_num += 1
    for i + 1, name in enumerate(spike_counters):
